...
typedef struct {
    ...
    XE2e_system_uio_map maps[ MAX_UIO_MAPS ];
} XE2e_system_uio_info;

/***************** Variable Definitions **************************************/
static XE2e_system_uio_info uio_info;
...
static int uio_info_read_name(XE2e_system_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XE2e_system_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XE2e_system_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XE2e_system_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XE2e_system_Initialize(XE2e_system *InstancePtr, const char* InstanceName) {
	XE2e_system_uio_info *InfoPtr = &uio_info;
